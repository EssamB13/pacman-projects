...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        self.particles = [] 
        evenParticleNum = self.numParticles / len(self.legalPositions) 
        for pos in self.legalPositions:
            for i in range(evenParticleNum):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for pos in self.legalPositions:
                ...
                allPossible[pos] = allPossible[pos] + emissionModel[dist] * oldBeliefs[pos]
            #all particles with weight zero
            if not any(allPossible.values()):
                self.initializeUniformly(gameState)
            else:
                holding = []
                for i in range (0, self.numParticles):
                    holding.append(util.sample(allPossible))
                self.particles = holding
...
class JointParticleFilter:
    ...
    def observeState(self, gameState):
        ...
        #particles need to be reinitialized
        if not any(allPossible.values()):
            self.initializeParticles()
            for ind, particle in enumerate(self.particles):
                for i in range(self.numGhosts):
