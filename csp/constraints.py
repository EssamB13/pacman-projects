...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        for x in qi.domain():
            for y in qj.domain():
                if x != y and not (abs(x-y) == abs(i - j)):
                    self.satAssignments.append([x,y])
...
class NValuesConstraint(Constraint):
    ...
    def check(self):
        ...
        return self._lb <= len(assignments) and len(assignments) <= self._ub
    ...
    def hasSupport(self, var, val):
        ...
        def valsNotNValues(l):
            '''tests a list of assignments which are pairs (var,val)
               to see if they can satisfy the NValues Constraint'''
            vals = [value for (var, value) in l]
            return self._lb <= vals.count(self._required) and vals.count(self._required) <= self._ub
        
        varsToAssign = self.scope()
        varsToAssign.remove(var)
        x = findvals(varsToAssign, [(var, val)], valsNotNValues, valsNotNValues)
        return x
